...
export async function GET() {
  ...
  const rows = getDb()
    .prepare(
      "SELECT id, provider, label, last_polled, last_error, created_at FROM backup_connections WHERE school_id = ? ORDER BY created_at DESC"
    )
    .all(schoolId);
  return NextResponse.json(rows);
}

// POST — create a new backup connection
...
// DELETE — remove a connection and all its jobs
export async function DELETE(req: NextRequest) {
  ...
  getDb().prepare("DELETE FROM backup_jobs WHERE connection_id = ? AND school_id = ?").run(body.id, schoolId);
  getDb().prepare("DELETE FROM backup_connections WHERE id = ? AND school_id = ?").run(body.id, schoolId);
