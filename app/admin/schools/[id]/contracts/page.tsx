...
function ExpiryBadge({ dateStr }: { dateStr: string | null }) {
  ...
  if (days < 0) return <span className="text-xs font-medium text-red-700 bg-red-100 px-2 py-0.5 rounded-full">{label} (Expired)</span>;
  if (days <= 30) return <span className="text-xs font-medium text-red-700 bg-red-100 px-2 py-0.5 rounded-full">{label} ({days}d)</span>;
...
export default function ContractsPage() {
  ...
  async function load() {
    const [school, rows] = await Promise.all([
      ...
      fetch(`/api/admin/schools/${schoolId}/contracts`).then(r => r.json()),
    ]);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const url = `/api/admin/schools/${schoolId}/contracts`;
  ...
  async function handleDelete(id: string, name: string) {
    ...
    await fetch(`/api/admin/schools/${schoolId}/contracts`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl border p-4 text-center shadow-sm">
            <div className={`text-2xl font-bold ${contracts.filter(c => daysUntil(c.end_date) !== null && daysUntil(c.end_date)! < 0).length > 0 ? "text-red-600" : "text-gray-400"}`}>
              {contracts.filter(c => daysUntil(c.end_date) !== null && daysUntil(c.end_date)! < 0).length}
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="flex items-center gap-2 pt-4">
                  <input type="checkbox" id="auto_renew" checked={!!form.auto_renew} onChange={e => setForm(f => ({ ...f, auto_renew: e.target.checked ? 1 : 0 }))} className="rounded" />
              ...
              <tbody className="divide-y">
                {contracts.map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-500 text-xs">{c.start_date ? new Date(c.start_date).toLocaleDateString("en-GB") : "—"}</td>
