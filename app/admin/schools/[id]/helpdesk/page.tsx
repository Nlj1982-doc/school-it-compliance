...
export default function AdminHelpdeskPage() {
  ...
  async function load() {
    const [school, rows] = await Promise.all([
      ...
      fetch(`/api/admin/schools/${schoolId}/helpdesk`).then(r => r.json()),
    ]);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/admin/schools/${schoolId}/helpdesk`, {
  ...
  async function handleStatusChange(ticket: Ticket, newStatus: string) {
    await fetch(`/api/admin/schools/${schoolId}/helpdesk`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
