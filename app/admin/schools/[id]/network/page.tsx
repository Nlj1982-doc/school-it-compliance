...
function ExpiryBadge({ dateStr, label = "" }: { dateStr: string | null; label?: string }) {
  ...
  const text = label ? `${label}: ${fmt}` : fmt;
  if (days < 0) return <span className="text-xs font-medium text-red-700 bg-red-100 px-2 py-0.5 rounded-full">{text} (Expired)</span>;
  if (days <= 30) return <span className="text-xs font-medium text-red-700 bg-red-100 px-2 py-0.5 rounded-full">{text} ({days}d)</span>;
  if (days <= 90) return <span className="text-xs font-medium text-yellow-700 bg-yellow-100 px-2 py-0.5 rounded-full">{text} ({days}d)</span>;
...
export default function AdminNetworkPage() {
  ...
  async function load() {
    const [school, rows] = await Promise.all([
      ...
      fetch(`/api/admin/schools/${schoolId}/network`).then(r => r.json()),
    ]);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/admin/schools/${schoolId}/network`, {
  ...
  async function handleDelete(id: string, name: string) {
    ...
    await fetch(`/api/admin/schools/${schoolId}/network`, {
              ...
              <table className="w-full text-xs">
                <thead className="bg-gray-50 sticky top-0">
                  <tr className="border-b text-gray-500">
                    {["#","Type","Tag","Name","Make/Model","IP","MAC","VLAN","Location","Warranty End","Status","Issue"].map(h => (
              ...
              <div className="border-t pt-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  <div className="sm:col-span-2"><label className="block text-xs font-medium text-gray-600 mb-1">Management URL</label>
                    <input type="text" value={form.management_url??""} onChange={set("management_url")} placeholder="e.g. https://192.168.1.2 or https://unifi.local"
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
        ...
        <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">
              {devices.length === 0 ? "No network devices yet. Click + Add Device to get started." : "No devices match your filters."}
            </div>
          ) : (
                  ...
                  {filtered.map(d => (
                    <tr key={d.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        ...
                        {!d.ip_address && !d.mac_address && <span className="text-gray-300">—</span>}
                      </td>
