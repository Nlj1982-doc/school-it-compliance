...
function poolStatus(item: PoolItem, loans: Loan[]): { status: "Available" | "On Loan" | "Overdue"; loan: Loan | null } {
  ...
  const s = loanStatus(active);
  return { status: s === "Overdue" ? "Overdue" : "On Loan", loan: active };
}
...
export default function LoansPage() {
  ...
  async function load() {
    const [p, l] = await Promise.all([
      fetch("/api/my/loan-pool").then(r => r.json()),
  ...
  async function handlePoolSave(e: React.FormEvent) {
    ...
    const res = await fetch("/api/my/loan-pool", {
  ...
  async function handleLoanOut(e: React.FormEvent) {
    ...
    if (!res.ok) { setLoanOutError((await res.json()).error ?? "Failed to record loan"); return; }
  ...
  function startNewLoan() {
    setLoanForm({ ...LOAN_EMPTY, date_out: new Date().toISOString().slice(0, 10) });
    setEditingLoanId("new"); setLoanError("");
  }
  ...
  async function handleLoanDelete(id: string, name: string) {
    ...
    await fetch("/api/my/loans", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
    load();
  }
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-1 bg-white border rounded-xl p-1 w-fit shadow-sm mb-6">
          <button onClick={() => setTab("pool")}
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${tab === "pool" ? "bg-blue-700 text-white" : "text-gray-600 hover:bg-gray-100"}`}>
            Equipment Pool {pool.length > 0 && <span className="ml-1 opacity-70">({pool.length})</span>}
          </button>
          <button onClick={() => setTab("history")}
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${tab === "history" ? "bg-blue-700 text-white" : "text-gray-600 hover:bg-gray-100"}`}>
        ...
        {tab === "pool" && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-gray-500">Manage your loanable equipment. Click <strong>Loan Out</strong> to record when an item is borrowed.</p>
              <button onClick={startNewPoolItem} className="bg-blue-700 hover:bg-blue-800 text-white px-4 py-2 rounded-lg text-sm font-medium">
                + Add to Pool
              </button>
            </div>
                ...
                <form onSubmit={handlePoolSave} className="space-y-4">
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    <div className="sm:col-span-2">
                      ...
                      <input type="text" value={poolForm.name} onChange={pSet("name")} required placeholder="e.g. Dell Laptop 01, iPad #3"
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Device Type</label>
                      <select value={poolForm.device_type} onChange={pSet("device_type")}
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <div>
                      ...
                      <input type="text" value={poolForm.make} onChange={pSet("make")} placeholder="e.g. Dell, Apple"
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <input type="text" value={poolForm.notes} onChange={pSet("notes")} placeholder="Any relevant notes…"
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                  {poolError && <div className="text-red-600 text-sm bg-red-50 border border-red-200 px-3 py-2 rounded-lg">{poolError}</div>}
                  <div className="flex gap-2">
                    <button type="submit" disabled={poolSaving} className="bg-blue-700 hover:bg-blue-800 text-white px-5 py-2 rounded-lg text-sm font-medium disabled:opacity-50">
                    ...
                    <div key={item.id} className={`bg-white rounded-xl border shadow-sm p-4 flex flex-col gap-3 ${status === "Overdue" ? "border-red-200" : ""}`}>
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-gray-500 mt-0.5">
                            {[item.device_type, item.make, item.model].filter(Boolean).join(" · ") || "—"}
                          </div>
                      ...
                      {loan && (
                        <div className="bg-gray-50 rounded-lg px-3 py-2 text-xs text-gray-600 space-y-0.5">
                          ...
                          {loan.date_due && (
                            <div className={loanStatus(loan) === "Overdue" ? "text-red-600 font-medium" : ""}>
                              Due: {new Date(loan.date_due).toLocaleDateString("en-GB")}
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      <div className="flex gap-2 mt-auto pt-2 border-t flex-wrap">
                        {status === "Available" ? (
                          <button onClick={() => startLoanOut(item)}
                            className="flex-1 bg-blue-700 hover:bg-blue-800 text-white px-3 py-1.5 rounded-lg text-xs font-medium text-center">
                            Loan Out
                          </button>
                        ) : (
                          <button onClick={() => loan && setReturnState({ loanId: loan.id, date: new Date().toISOString().slice(0, 10), condition: "" })}
                            className="flex-1 bg-green-700 hover:bg-green-800 text-white px-3 py-1.5 rounded-lg text-xs font-medium text-center">
                            Mark Returned
                          </button>
                        )}
        ...
        {tab === "history" && (
          <div>
            <div className="flex flex-wrap gap-3 items-center justify-between mb-4">
              ...
              <button onClick={startNewLoan} className="bg-blue-700 hover:bg-blue-800 text-white px-4 py-2 rounded-lg text-sm font-medium">
                + Ad-hoc Loan
              </button>
            </div>
                ...
                <form onSubmit={handleLoanSave} className="space-y-4">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <div>
                      ...
                      <select value={loanForm.borrower_type} onChange={lSet("borrower_type")}
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <input type="text" value={loanForm.equipment} onChange={lSet("equipment")} required placeholder="e.g. Dell Laptop, iPad"
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Asset Tag</label>
                    ...
                    <div>
                      ...
                      <input type="date" value={loanForm.date_out} onChange={lSet("date_out")} required
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Expected Return</label>
                      <input type="date" value={loanForm.date_due ?? ""} onChange={lSet("date_due")}
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Condition Out</label>
                      <select value={loanForm.condition_out ?? ""} onChange={lSet("condition_out")}
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="">— Select —</option>
                    ...
                    <div>
                      ...
                      <input type="date" value={loanForm.date_returned ?? ""} onChange={lSet("date_returned")}
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Condition In</label>
                      <select value={loanForm.condition_in ?? ""} onChange={lSet("condition_in")}
                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="">— Select —</option>
                        {CONDITIONS.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                  ...
                  <div className="flex gap-2">
                    <button type="submit" disabled={loanSaving} className="bg-blue-700 hover:bg-blue-800 text-white px-5 py-2 rounded-lg text-sm font-medium disabled:opacity-50">
            ...
            <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
              {loading ? (
                ...
                <div className="p-8 text-center text-gray-400">
                  {loans.length === 0 ? "No loan records yet." : "No loans match the current filter."}
                </div>
              ) : (
                      ...
                      {filteredLoans.map(l => (
                        <tr key={l.id} className={`hover:bg-gray-50 ${loanStatus(l) === "Overdue" ? "bg-red-50" : ""}`}>
                          ...
                          <td className="px-4 py-3 text-gray-600 text-xs">
                            {l.date_due ? <span className={loanStatus(l) === "Overdue" ? "text-red-600 font-medium" : ""}>{new Date(l.date_due).toLocaleDateString("en-GB")}</span> : "—"}
                          </td>
                          ...
                          <td className="px-4 py-3 whitespace-nowrap">
                            {!l.date_returned && (
                              <button onClick={() => setReturnState({ loanId: l.id, date: new Date().toISOString().slice(0, 10), condition: "" })}
                                className="text-green-600 hover:text-green-800 text-xs font-medium mr-3">Mark Returned</button>
                            )}
            ...
            <form onSubmit={handleLoanOut} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  ...
                  <input type="text" value={loanOutState.borrower_name} onChange={loSet("borrower_name")} required
                    placeholder="e.g. James Smith"
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Type</label>
                  <select value={loanOutState.borrower_type} onChange={loSet("borrower_type")}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    {BORROWER_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <input type="date" value={loanOutState.date_out} onChange={loSet("date_out")} required
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Expected Return</label>
                  <input type="date" value={loanOutState.date_due} onChange={loSet("date_due")}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Condition Out</label>
                  <select value={loanOutState.condition_out} onChange={loSet("condition_out")}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="">— Select —</option>
                    {CONDITIONS.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
              ...
              <div className="flex gap-2 pt-1">
                <button type="submit" disabled={loanOutSaving} className="flex-1 bg-blue-700 hover:bg-blue-800 text-white py-2 rounded-lg text-sm font-medium disabled:opacity-50">
                  {loanOutSaving ? "Recording…" : "Confirm Loan Out"}
                </button>
